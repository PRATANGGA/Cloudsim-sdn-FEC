 ...
 */
package org.cloudbus.cloudsim.sdn.example.fuzzycmeans;

import static org.cloudbus.cloudsim.sdn.example.fuzzycmeans.FCMInterface.DATA_ATTR;
import static org.cloudbus.cloudsim.sdn.example.fuzzycmeans.FCMInterface.ERR;
import static org.cloudbus.cloudsim.sdn.example.fuzzycmeans.FCMInterface.MAX_ITER;
 ...
 * @author Nobita-Kun
 */
public class FCM_5C extends  PowerVmSelectionPolicy{
    ...
    ArrayList<DataSet> DataKluster5 = new ArrayList();
    ...
    public void showEachCLuster ()
    {
        ...
        if (!DataKluster5.isEmpty())
        {
            System.out.println("===== DATA PADA KLUSTER 5 =====");
            fcm.showData(DataKluster5);
            System.out.println("\tRata2 Kluter 5 = ["+fcm.getAverageColumn(DataKluster5,0)+"] , ["+fcm.getAverageColumn(DataKluster5,1)+"]");
        }
    ...
    public void clusterSelection ()
    {
        while (!alreadygetVM)
        {
            ...
            else if (currentCluster==5 && !DataKluster5.isEmpty())
            {
                if (FCMInterface.SHOW_DEBUG)
                {
                    System.out.println("\n > VM dengan ID = "+DataKluster5.get(0).getIndexVM()+" , pada Kluster 5 telah dipindahkan");
                }
                
                moveVMID = DataKluster5.get(0).getIndexVM();
                DataKluster5.remove(0);
                alreadygetVM = true;
            }
         
            
            currentCluster = (currentCluster + 1) % 6;
            if (currentCluster==0)
            {
                currentCluster++;
            }
...
//****************************** AREA PROSES FCM-NYA **********************************//
                            ...
                            DataKluster5 = fcm.groupDataCluster(dataVM,5);
